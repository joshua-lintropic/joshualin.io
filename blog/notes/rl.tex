...
\begin{concept}[Policy Optimization]
    ...
    \begin{maxi*}
        {\theta}
        {J(\pi_\theta) = \mathbb{E}_{\tau \sim \pi_\theta}[R(\tau)] = \mathbb{E}_{s \sim \mu}[V^{\pi_\theta}(s)]}
        {}
        {}
    \end{maxi*}
...
\begin{concept}[Discounted State Occupancy Measure (DSOM)]
    ...
    \]
    An important special case arises from our existing identities for \(J(\pi_\theta)\) and \(\nabla_\theta J(\pi_\theta)\): 
    \begin{align*}
        J(\pi_\theta) &= (1 - \gamma) \mathbb{E}_{s \sim d^{\pi_\theta},\, a \sim \pi_\theta(\cdot \mid s)}[r(s, a)] \\
        \nabla_\theta J(\pi_\theta) &= (1 - \gamma) \mathbb{E}_{s \sim d^{\pi_\theta},\, a \sim \pi_\theta(\cdot \mid s)}[A^{\pi_\theta}(s, a) \nabla_\theta \log \pi_\theta(a \mid s)]
    \end{align*}
    The DSOM can be extended to continuous state spaces as well. Notably, the expressions for \(J(\pi_\theta)\) and \(\nabla_\theta J(\pi_\theta)\) remain identical. 
\end{concept}
...
\begin{concept}[Natural Policy Gradient (Kakade, 2001)]
    ...
    \begin{maxi*}
        {\Delta\theta}
        {\nabla_\theta J(\pi_\theta)^\top \Delta\theta} 
        {}
        {}
    ...
    \[
        \widetilde{\nabla}_\theta J(\pi_\theta) = \mathcal{I}(\pi_\theta)^{-1} \nabla_\theta J(\pi_\theta)
    \]
    with optimal step size 
    \[
        \alpha = \sqrt{\frac{2\delta}{\nabla_\theta J(\pi_\theta)^\top \mathcal{I}(\pi_\theta)^{-1} \nabla_\theta J(\pi_\theta)}}
    \]
...
\begin{concept}[Conservative Policy Iteration (Kakade \& Langford, 2002)]
    ...
    \[
        ...
        = J(\pi_\theta) + \frac{1}{1 - \gamma} \mathbb{E}_{s \sim d^{\pi_\theta},\, a \sim \pi_\theta(\cdot \mid s)} \left[ \frac{\pi_{\theta'}(a \mid s)}{\pi_\theta(a \mid s)} A^{\pi_\theta}(s, a) \right]
    \]
    Often we drop the \(J(\pi_\theta)\) and \((1 - \gamma)^{-1}\) since they don't impact the optimization. Notably \(\nabla_\theta J(\pi_\theta) = \nabla_{\theta'} L_{\pi_\theta}(\pi_{\theta'}) \rvert_{\theta' = \theta}\), so \(L_{\pi_\theta}(\pi_{\theta'})\) and \(J(\pi_{\theta'})\) agree to first-order. If the update returns a \textit{mixture policy} \(\pi_{\theta'} = (1 - \alpha^*) \pi_\theta + \alpha^* \argmax_{\pi} L_{\pi_\theta}(\pi)\) for some optimal mixing parameter \(\alpha^* \in [0,1]\) defined by 
    \[
    ...
    \[
        J(\pi_{\theta'}) \geq L_{\pi_\theta}(\pi_{\theta'}) - \frac{2\gamma\varepsilon\alpha^2}{(1 - \gamma)^2}
        = J(\pi_\theta) + \alpha \mathbb{E}_{s \sim d^{\pi_\theta},\, a \sim \pi_\theta(\cdot \mid s)}\left[ \frac{\pi_{\theta'}(a \mid s)}{\pi_\theta(a \mid s)} A^{\pi_\theta}(s, a)\right] - \frac{2\gamma\varepsilon\alpha^2}{(1 - \gamma)^2}
    \]
...
\begin{concept}[Trust Region Policy Optimization (Schulman et. al., 2015)]
    ...
    \[
        J(\pi_{\theta'}) \geq L_{\pi_\theta}(\pi_{\theta'}) - \frac{4\varepsilon\gamma}{(1 - \gamma)^2} \max_{s \in \mathcal{S}} \KL{\pi_\theta(\cdot \mid s)}{\pi_{\theta'}(\cdot \mid s)}
    \]
...
\begin{concept}[Generalized Advantage Estimation (Schulman et. al., 2016)]
    GAE provides a family of advantage estimators that interpolate between low-bias, high-variance Monte Carlo estimates and high-bias, low-variance bootstrapped estimates. Define the \textit{temporal difference (TD) residual} as 
    \[
        \delta_t^V = r(s_t, a_t) + \gamma V(s_{t+1}) - V(s_t)
    \]
    where \(V \approx V^{\pi_\theta}\) is a learned value function. Note that \(\mathbb{E}_{\tau \sim \pi_\theta}[\delta_t^V \mid s_t, a_t] = A^{\pi_\theta}(s_t, a_t)\) when \(V = V^{\pi_\theta}\), so \(\delta_t^V\) is an unbiased one-step estimate of the advantage. The \textit{GAE} is an exponentially-weighted average of multi-step advantage estimates: 
    \[
        \hat{A}_t^{\text{GAE}(\gamma, \lambda)} = \sum_{l=0}^{T-t} (\gamma\lambda)^l \delta_{t+l}^V
    \]
    where \(\lambda \in [0,1]\) is the \textit{bias-variance tradeoff parameter}. Notably: 
    \begin{itemize}
        \item \(\lambda = 0\): \(\hat{A}_t = \delta_t^V = r(s_t, a_t) + \gamma V(s_{t+1}) - V(s_t)\), the one-step TD residual (high bias, low variance). 
        \item \(\lambda = 1\): \(\hat{A}_t = \sum_{l=0}^{T-t} \gamma^l \delta_{t+l}^V = R_t(\tau) - V(s_t)\), the Monte Carlo advantage (low bias, high variance). 
    \end{itemize}
\end{concept}

\begin{remark}
    The \(\lambda = 1\) identity follows by expanding the TD residuals: 
    \begin{align*}
        \sum_{l=0}^{T-t} \gamma^l \delta_{t+l}^V 
        &= \sum_{l=0}^{T-t} \gamma^l \left( r(s_{t+l}, a_{t+l}) + \gamma V(s_{t+l+1}) - V(s_{t+l}) \right) \\
        &= \sum_{l=0}^{T-t} \gamma^l r(s_{t+l}, a_{t+l}) + \sum_{l=0}^{T-t} \gamma^{l+1} V(s_{t+l+1}) - \sum_{l=0}^{T-t} \gamma^l V(s_{t+l})
    \end{align*}
    The last two sums telescope (reindex the first as \(\sum_{l=1}^{T-t+1} \gamma^l V(s_{t+l})\)), leaving \(\gamma^{T-t+1} V(s_{T+1}) - V(s_t)\). For a terminal state \(V(s_{T+1}) = 0\), or in the infinite-horizon discounted setting where the tail vanishes, the result is \(R_t(\tau) - V(s_t)\). Intermediate values of \(\lambda\) smoothly interpolate; in practice, \(\lambda \in [0.95, 0.99]\) is standard.
\end{remark}
...
\begin{concept}[Actor-Critic]
    ...
    \[
        \nabla_\theta J(\pi_\theta) \approx \mathbb{E}_{\tau \sim \pi_\theta} \left[ \sum_{t=0}^T \gamma^t \hat{A}_t^{\text{GAE}} \nabla_\theta \log \pi_\theta(a_t \mid s_t) \right]
    \]
    The critic is updated by minimizing the squared error loss \(\mathcal{L}(\phi) = \mathbb{E}_{\tau \sim \pi_\theta}\left[\sum_{t=0}^T (V_\phi(s_t) - R_t(\tau))^2\right]\). Unlike REINFORCE, the actor-critic framework allows for lower-variance gradient estimates at the cost of bias introduced by the learned value function. 
\end{concept}
...
\begin{concept}[Proximal Policy Optimization (Schulman et. al., 2017)]
    ...
    \]
    The \textbf{PPO-Clip} objective is 
    \[
        L^{\text{CLIP}}(\theta') = \mathbb{E}_{\tau \sim \pi_\theta} \left[ \sum_{t=0}^T \min\left( \rho_t(\theta') \hat{A}_t,\; \text{clip}(\rho_t(\theta'), 1-\varepsilon, 1+\varepsilon) \hat{A}_t \right) \right]
    \]
    where \(\hat{A}_t = \hat{A}_t^{\text{GAE}(\gamma,\lambda)}\) and \(\varepsilon > 0\) (typically \(\varepsilon = 0.2\)) is the clipping hyperparameter. The clipping function is 
    \[
        \text{clip}(\rho, 1-\varepsilon, 1+\varepsilon) = \max(1-\varepsilon, \min(\rho, 1+\varepsilon))
    \]
    which truncates the ratio to the interval \([1-\varepsilon, 1+\varepsilon]\). The full PPO loss, combining the actor, critic, and an entropy bonus for exploration, is 
    \[
        \mathcal{L}^{\text{PPO}}(\theta', \phi) = -L^{\text{CLIP}}(\theta') + c_1 \mathcal{L}^{\text{VF}}(\phi) - c_2 H(\pi_{\theta'})
    \]
    where \(\mathcal{L}^{\text{VF}}(\phi) = \mathbb{E}_{\tau \sim \pi_\theta}\left[\sum_{t=0}^T (V_\phi(s_t) - R_t(\tau))^2\right]\) is the value function loss, \(H(\pi_{\theta'}) = \mathbb{E}_{s \sim d^{\pi_\theta}}[H(\pi_{\theta'}(\cdot \mid s))]\) is the mean entropy of the policy, and \(c_1, c_2 > 0\) are weighting coefficients. 
\end{concept}

\begin{remark}
    \textbf{Mechanics of Clipping.} The clipped objective is best understood by case analysis on the sign of \(\hat{A}_t\): 
    \begin{itemize}
        \item \(\hat{A}_t > 0\) (action was better than expected): increasing \(\rho_t\) improves the surrogate, but clipping caps the benefit at \(\rho_t = 1 + \varepsilon\). The minimum selects the more conservative (smaller) term, so 
        \[
            \min(\rho_t \hat{A}_t, (1+\varepsilon)\hat{A}_t) = \min(\rho_t, 1+\varepsilon) \hat{A}_t
        \]
        preventing the policy from moving too aggressively toward good actions. 
        \item \(\hat{A}_t < 0\) (action was worse than expected): decreasing \(\rho_t\) improves the surrogate, but clipping caps the benefit at \(\rho_t = 1 - \varepsilon\). Here, 
        \[
            \min(\rho_t \hat{A}_t, (1-\varepsilon)\hat{A}_t) = \max(\rho_t, 1-\varepsilon) \hat{A}_t
        \]
        preventing the policy from moving too far from bad actions. 
    \end{itemize}
    In both cases, the gradient is zero outside the trust region \(\rho_t \in [1-\varepsilon, 1+\varepsilon]\), providing a ``soft wall'' analogous to TRPO's hard KL constraint. Crucially, the clipped objective is a \textit{pessimistic} (lower) bound on the unclipped surrogate, so the optimization never exploits overconfident advantage estimates.

    \textbf{Notes.} We discuss several important details. 
    \begin{itemize}
        \item \textit{Minibatch updates.} Unlike TRPO (which performs a single constrained step per batch of data), PPO performs multiple epochs of minibatch SGD on the same batch of trajectories. This is the primary source of its sample efficiency gains. The clipping mechanism ensures that each epoch does not stray too far from the data-collecting policy, even without an explicit constraint. 
        \item \textit{Entropy bonus.} The term \(c_2 H(\pi_{\theta'})\) encourages exploration by penalizing deterministic policies. Without it, the policy can prematurely converge to a suboptimal deterministic action. This is especially important in environments with sparse or deceptive rewards.
...
\begin{concept}[PPO-Clip Algorithm]
    ...
    \For{\(iteration = 0, \dots, \text{MAX\_ITER}\)}
        ...
        \State Compute advantages \(\hat{A}_t^{\text{GAE}(\gamma,\lambda)}\) for all \((s_t, a_t) \in \mathcal{D}\) using \(V_\phi\)
        \State Normalize advantages: \(\hat{A}_t \leftarrow (\hat{A}_t - \mu_{\hat{A}}) / \sigma_{\hat{A}}\) over the batch
        \For{\(epoch = 1, \dots, K\)}
            \For{each minibatch \(\mathcal{B} \subset \mathcal{D}\) of size \(M\)}
                ...
                \State Compute clipped surrogate: \(L^{\text{CLIP}} = \frac{1}{|\mathcal{B}|}\sum_{t \in \mathcal{B}} \min(\rho_t \hat{A}_t, \text{clip}(\rho_t, 1{-}\varepsilon, 1{+}\varepsilon)\hat{A}_t)\) 
                ...
                \State \((\theta', \phi) \leftarrow (\theta', \phi) - \alpha \nabla_{(\theta', \phi)} (-L^{\text{CLIP}} + c_1 \mathcal{L}^{\text{VF}} - c_2 H)\)
            \EndFor
        \EndFor
...
\begin{remark}
    ...
    \[
        L^{\text{KL}}(\theta') = \mathbb{E}_{\tau \sim \pi_\theta}\left[\sum_{t=0}^T \rho_t(\theta') \hat{A}_t\right] - \beta \TKL{\pi_\theta}{\pi_{\theta'}}
    \]
