...
\begin{concept}[Deep Q Networks (Mnih et. al., 2015)]
    ...
    \end{itemize}
    The loss minimized is the mean squared Bellman error over minibatches from the replay buffer,
    \[
        \mathcal{L}(\phi) = \mathbb{E}_{(s,a,r,s') \sim \mathcal{D}} \left[ \left( r + \gamma \max_{a'} Q_{\phi^-}(s', a') - Q_\phi(s, a) \right)^2 \right]
    \]
    The agent acts \(\varepsilon\)-greedily with respect to \(Q_\phi\), typically with \(\varepsilon\) annealed from \(1.0\) to \(0.1\) over training.
\end{concept}
